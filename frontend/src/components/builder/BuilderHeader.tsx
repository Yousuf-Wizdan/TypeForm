...
export const BuilderHeader: React.FC<BuilderHeaderProps> = ({
  ...
  return (
    <header className="sticky top-0 z-30 flex h-[60px] w-full items-center justify-between border-b border-[var(--border-default)] bg-[var(--background)]/90 backdrop-blur-xl px-4 lg:px-6">
      {/* Left */}
        ...
        <div className="flex items-center gap-2.5 min-w-0">
          {isEditingTitle ? (
            <input
              ...
              autoFocus
              className="rounded-xl border border-[var(--border-strong)] bg-[var(--surface)] px-3 py-1.5 text-sm font-semibold text-[var(--text-primary)] focus:outline-none focus:ring-2 focus:ring-[var(--brand-primary)] focus:ring-offset-1"
            />
          ) : (
            <h1
              onClick={() => setIsEditingTitle(true)}
              className="cursor-pointer truncate text-sm font-semibold text-[var(--text-primary)] hover:text-[var(--text-muted)] transition-colors"
